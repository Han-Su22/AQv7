...
"""
'''
Run in sequence! Run lowinput first to set up calibration
Yield under soil fertility stress for low-input rainfed, real
virtual_irrigation='Lowinput'
scenarios = [1,2]
soil_fertility=1

Yield under soil fertility stress if no water stress for low-input rainfed locations
virtual_irrigation='Lowvirt'
scenarios = [1,2]
soil_fertility=1

Yield under no soil fertility stress if no water stress for low-input rainfed locations
virtual_irrigation='Highvirt'
scenarios = [1,2]
soil_fertility=0

Yield under no soil fertility stress for high-input rainfed and irrigated, real
...
class project_conf:
    # General
    project_name = "Maize" #cannot have _
    crop_model = 'AquaCrop_v6' # Only this one available for now
    scenarios = [1,2,3,4]  # Scenarios to consider, for noSF [1,2,3,4]
    multi_core = True
    CPUs = 7
    ...
    # Crop settings
    crop_name = 'Maize' # Full name of a crop
    crop_name_short = 'mai' # Crop's code name from GGCMI
    crop_fao = 56 # Crop's code from FAO
    crop_gs_increase = 15 # % to prolong the growing season length if maturity is not reached
    ...
    crop_name_4code='maiz'#SPAM crop
    #C:\Users\SuH\OneDrive - Universiteit Twente\WaterResearch\AquaCrop\GAEZ\Crop_table.xlsx
    crop_gaez=['Highland maize','Lowland maize','Maize','Temperate maize']#only useful for soil fertility calibration
    #a*(1-np.exp(b*LAI))**c
    LAI_CC_a=1.005
    LAI_CC_b=-0.6
    LAI_CC_c=1.2
