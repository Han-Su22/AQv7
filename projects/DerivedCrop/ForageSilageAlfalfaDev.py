...
class project_conf:
    # General
    project_name = "ForageSilageAlfalfa"
    ...
    # Crop settings
    crop_name = 'ForageSilageAlfalfa' # Full name of a crop
    crop_name_short = 'aff' # Crop's code name from GGCMI
    crop_fao = 641 # Crop's code from FAO
    crop_gs_increase = 15 # % to prolong the growing season length if maturity is not reached
    crop_perennial = True
    crop_phenology = False
    
    # Deriving from
    cor_gen_crop_fao = 641 # Corresponding generic FAO crop code 
    cor_gen_crop_name_short = 'aff' # Corresponding generic crop's code name (3-letter format)
    ...
    crop_name_4code='fodder_crops'#SPAM crop
    #C:\Users\SuH\OneDrive - Universiteit Twente\WaterResearch\AquaCrop\GAEZ\Crop_table.xlsx
    crop_gaez=['Alfalfa']#only useful for soil fertility calibration
    #a*(1-np.exp(b*LAI))**c
    LAI_CC_a=1
    LAI_CC_b=-0.8
    LAI_CC_c=1.
