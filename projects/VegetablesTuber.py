...
class project_conf:
    ...
    # Crop settings
    crop_name = 'VegetablesTuber' # Full name of a crop
    crop_name_short = 'vgt' # Crop's code name from GGCMI
    crop_fao = 10001 # Crop's code from FAO
    ...
    # Soil settings
    soil_dz = [.1, .1, .1, .3, .4, .5, .5] # Soil compartments [m], 2m in total
    init_wc = 50 # Initital soil moisture [%]
    gw_max_level = -.5 # Limit shallow groundwater to this number (only for scenarios with groundwater)
    gw_min_level = -2. # Skip the cells with groundwater levels below this (only for scenarios with groundwater)
    
    # Field management settings
    off_season = True # Consider or not fallow period (affects soil moisture)
    irr_thresholds = [60,60,60,60] # % of TAW when to trigger irrigation events
    ...
    crop_name_4code='vege'#SPAM crop
    #C:\Users\SuH\OneDrive - Universiteit Twente\WaterResearch\AquaCrop\GAEZ\Crop_table.xlsx
    crop_gaez=['White potato']#only useful for soil fertility calibration
    #a*(1-np.exp(b*LAI))**c
    LAI_CC_a=1
    LAI_CC_b=-0.5808
    LAI_CC_c=1.
    get_GAEZ_ccx_hi=False
