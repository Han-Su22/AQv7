...
class project_conf:
    ...
    # Crop settings
    crop_name = 'Groundnut' # Full name of a crop
    crop_name_short = 'nut' # Crop's code name from GGCMI
    crop_fao = 242 # Crop's code from FAO
    ...
    irr_thresholds = [40, 40, 40, 40] # % of TAW when to trigger irrigation events
    ...
    crop_name_4code='grou'#SPAM crop
    #C:\Users\SuH\OneDrive - Universiteit Twente\WaterResearch\AquaCrop\GAEZ\Crop_table.xlsx
    crop_gaez=['Groundnut']#only useful for soil fertility calibration
    #a*(1-np.exp(b*LAI))**c
    LAI_CC_a=1
    LAI_CC_b=-0.6
    LAI_CC_c=1
