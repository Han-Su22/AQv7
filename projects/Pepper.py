...
class project_conf:
    ...
    # Crop settings
    crop_name = 'Pepper' # Full name of a crop
    crop_name_short = 'ppr' # Crop's code name from GGCMI
    crop_fao = 401 # Crop's code from FAO
    ...
    gw_max_level = -0.5 # Limit shallow groundwater to this number (only for scenarios with groundwater)
    gw_min_level = -2.0 # Skip the cells with groundwater levels below this (only for scenarios with groundwater)
    
    # Field management settings
    off_season = True # Consider or not fallow period (affects soil moisture)
    irr_thresholds = [65,65,65,65] # % of TAW when to trigger irrigation events
