...
class project_conf:
    ...
    # Crop settings
    crop_name = 'Chickpea' # Full name of a crop
    crop_name_short = 'ckp' # Crop's code name from GGCMI
    crop_fao = 191 # Crop's code from FAO
    ...
    irr_thresholds = [50, 50, 50, 50] # % of TAW when to trigger irrigation events
    ...
    crop_name_4code='chic'#SPAM crop
    #C:\Users\SuH\OneDrive - Universiteit Twente\WaterResearch\AquaCrop\GAEZ\Crop_table.xlsx
    crop_gaez=['Chickpea']#only useful for soil fertility calibration
    #a*(1-np.exp(b*LAI))**c
    LAI_CC_a=1
    LAI_CC_b=-0.8467
    LAI_CC_c=1
