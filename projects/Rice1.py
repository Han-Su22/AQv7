...
class project_conf:
    ...
    scenarios = [1,2,6]  # Scenarios to consider
    multi_core = True
    CPUs = 7
    ...
    # Crop settings
    crop_name = 'Rice1' # Full name of a crop
    crop_name_short = 'ri1' # Crop's code name from GGCMI
    crop_fao = 27 # Crop's code from FAO
    ...
    # Soil settings
    soil_dz = [.1, .1, .1, .3, .4, .5] # Soil compartments [m], 1.5m in total
    ...
    gw_min_level = -1.5 # Skip the cells with groundwater levels below this (only for scenarios with groundwater)
    
    # Field management settings
    off_season = True # Consider or not fallow period (affects soil moisture)
    irr_thresholds = [100]*4 # % of TAW when to trigger irrigation events
    bunds = True # Surface bunds are present
    ...
    crop_name_4code='rice'#SPAM crop
    #C:\Users\SuH\OneDrive - Universiteit Twente\WaterResearch\AquaCrop\GAEZ\Crop_table.xlsx
    crop_gaez=['Dryland rice','Wetland rice']#only useful for soil fertility calibration
    #a*(1-np.exp(b*LAI))**c, need to choose one which allows the maxium CC to our default setting
    LAI_CC_a=1
    LAI_CC_b=-0.535
    LAI_CC_c=1
