...
class project_conf:
    ...
    # Crop settings
    crop_name = 'Coffee' # Full name of a crop
    crop_name_short = 'cof' # Crop's code name from GGCMI
    crop_fao = 656 # Crop's code from FAO
    crop_gs_increase = 5 # % to prolong the growing season length if maturity is not reached
    crop_perennial = True
    ...
    crop_name_4code='rcof'#SPAM crop
    #C:\Users\SuH\OneDrive - Universiteit Twente\WaterResearch\AquaCrop\GAEZ\Crop_table.xlsx
    crop_gaez=['Coffee','Coffee robusta']#only useful for soil fertility calibration
    #a*(1-np.exp(b*LAI))**c
    LAI_CC_a=1
    LAI_CC_b=-0.506
    LAI_CC_c=1
